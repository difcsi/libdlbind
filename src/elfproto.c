...
{	
	...
	ehdr = (Elf64_Ehdr) {
		...
		.e_phnum = PHDRS_N /* text, data, rodata, dynamic, stack */,
	...
	phdrs[6] = (Elf64_Phdr) { // GNU_STACK - non-executable stack
			.p_type = PT_GNU_STACK,
			.p_flags = PF_R | PF_W, // readable and writable, but NOT executable
			.p_offset = 0,
			.p_vaddr = 0,
			.p_paddr = 0,
			.p_filesz = 0,
